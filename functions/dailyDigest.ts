...
async function pushAlertToAffinity(alert, ownerEmail) {
    ...
    try {
        ...
        if (!searchResponse.ok) {
            console.log(`⚠️ Failed to search Affinity for ${alert.company_name}: ${searchResponse.status}`);
...
`.trim();
        ...
        if (noteResponse.ok) {
            ...
            console.log(`⚠️ Failed to create note for ${alert.company_name}: ${errorText}`);
        ...
        if (isTier1Trigger) {
            ...
            if (taskResponse.ok) {
                ...
                console.log(`⚠️ Failed to create task for ${alert.company_name}: ${errorText}`);
